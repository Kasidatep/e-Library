...
<template>
    <div class="w-screen h-screen overflow-x-hidden">
        <div class="absolute w-screen h-[30rem] overflow-hidden object-center opacity-20">
            <img :src="(book.img?.length>10)?book.img:'../default/coverpost.png'" class="w-full h-fit object-center  bg-transparen z-0 blur-sm" alt="">
        </div>
        <div class="flex relative w-screen h-screen justify-center z-0">
            ...
            <div class=" mt-24 md:mt-40 mx-3 w-full md:w-3/5 rounded-t-3xl" :class="theme.bgbase">
                <div class=" z-0 " v-if="(book.img?.length>10)">
                    <img :src="book.img" class="w-full h-fit rounded-t-3xl z-0" alt="">
                </div>
                <div class="text-3xl pt-5 font-extrabold w-full text-center" :class="theme.textheader">{{ book.title }}
                </div>
